...
class LazyLoader {
    ...
    handleImageLoad(img) {
        ...
        // Dispatch event
        window.dispatchEvent(new CustomEvent('image-lazy-loaded', {
    ...
    refresh() {
        ...
        const newImages = document.querySelectorAll('img[data-src]:not(.lazy-loading):not(.lazy-loaded)');
        ...
        const newContent = document.querySelectorAll('[data-lazy-content]:not(.lazy-loading):not(.lazy-loaded)');
        ...
        if (newImages.length > 0 || newContent.length > 0) {
            console.log(`🔄 Refreshed lazy loading: ${newImages.length} images, ${newContent.length} content elements`);
        }
    }
    ...
    getPerformanceReport() {
        return {
            ...
            successRate: this.performanceMetrics.totalImages > 0 ?
                (this.performanceMetrics.loadedImages / this.performanceMetrics.totalImages * 100).toFixed(2) + '%' : '0%',
