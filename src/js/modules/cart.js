...
class CartManager {
  ...
  setState(newState) {
    ...
    window.dispatchEvent(
      new CustomEvent('cart-state-change', {
  ...
  handleError(message, error) {
    ...
    this.showNotification(
      `Something went wrong with your cart. Please try refreshing the page.`,
      'error'
    );
  ...
  updateAddToCartButtonStates() {
    try {
      document.querySelectorAll('.add-to-cart, .add-to-cart-btn').forEach(button => {
        const artworkId =
          button.getAttribute('data-id') || button.getAttribute('onclick')?.match(/'([^']+)'/)?.[1];
        if (artworkId) {
