...
class ErrorHandler {
    constructor() {
        ...
        this.userMessages = {
            [this.errorTypes.NETWORK]: 'Connection error. Please check your internet connection and try again.',
    ...
    handleComponentError(componentName, error, context = null) {
        ...
        if (boundary.errorCount >= boundary.options.maxErrors &&
    ...
    attemptComponentRecovery(boundary, error, forceRecover = false) {
        ...
        try {
            ...
            if (recovered) {
                ...
                window.dispatchEvent(new CustomEvent('component-recovered', {
    ...
    resetComponent(boundary) {
        ...
        try {
            ...
            const fallback = element.parentNode?.querySelector(`[data-error-fallback="${componentName}"]`);
            if (fallback) {
                fallback.remove();
            }
    ...
    showCriticalErrorMessage() {
        const message = 'Multiple errors detected. Please refresh the page or contact support if the problem persists.';
